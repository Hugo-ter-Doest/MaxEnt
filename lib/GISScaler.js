...
function GISScaler(classes, featureSet, sample) {
...
GISScaler.prototype.run = function(maxIterations, minImprovement) {
  ...
  var p = new Distribution(alpha, this.featureSet, this.classes, this.sample);
  ...
  do {
    ...
    for (var i = 0; i < this.featureSet.size(); i++) {
      ...
      expectationApprox = this.featureSet.getFeatures()[i].expectationApprox(p, this.classes, this.sample);
