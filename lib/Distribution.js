...
function Distribution(alpha, features, classes, sample) {
  this.alpha = alpha;
...
Distribution.prototype.weight = function(x) {
  ...
  this.alpha.forEach(function(alpha_j, j) {
    product *= Math.pow(alpha_j, that.features[j].apply(x));
  });
  return product;
};
...
Distribution.prototype.prepareAPosterioris = function() {
  ...
  this.sample.elements.forEach(function(sampleElement) {
    that.aPosterioriNormalisationConstants[sampleElement.b] =
      that.aPosterioriNormalisation(sampleElement.b);
  });
};
